...
Note that Alice's branch is **fast-forwarded** into `main`; it does not create a merge commit. The two-parent merge commit is created later by Bob's `git pull origin main`, which merges two divergent histories.
...
The exact label after `>>>>>>>` varies by merge source; after `git pull origin main` it typically names the remote-tracking ref such as `origin/main`.
...
```
```bash
git log main..feat/rebase-demo --oneline   # what is on the branch that is not on main (run after that branch exists)
```
```bash
...
git diff main..feat/rebase-demo       # branch tip vs branch tip (run after that branch exists)
git diff main...feat/rebase-demo      # branch vs their merge base (what the PR actually shows)
```
...
This is the moment to understand what force push does to your colleagues. A force push can leave collaborators' local and remote-tracking histories inconsistent with the remote. If the remote was moved backward to an ancestor, a collaborator whose clone still holds the removed commit may appear up to date locally, and a later push can reintroduce that commit. If the remote was rewritten to a genuinely divergent history, their next fetch or pull requires an explicit reconciliation strategy, and the commit you "removed" can come straight back, duplicated, with conflicts. In either case, stop all pushes, identify the intended remote tip, restore it if necessary, and tell collaborators how to realign their local branches. Any CI, any deployment, any tag pointing at the old SHA now dangles.
...
```
```bash
# Choose ONE recovery method. Each reset updates ORIG_HEAD, so do not run both in sequence.

# A. Explicit, and safest for a lab report:
git reset --hard <sha-from-reflog>            # you are back

# B. Immediately after the mistaken reset, while ORIG_HEAD still points at the old tip:
...
Lab 4D continues in this repository and uses GitHub's built-in issue types (Feature, Bug, Task), which are an **organization-only** feature. Create the repository under a GitHub organization you own (for example `module4-<username>`) so those types are available later; a free organization is sufficient. If you cannot use an organization, create it under your personal account and substitute an `epic` label for the issue type in Lab 4D.

```bash
gh repo create <org-or-username>/module4-release-demo --public --clone --add-readme
```
```bash
...
```
```bash
npm pkg set version=0.1.0     # npm init -y writes 1.0.0; pin 0.1.0 so the 0.x release sequence below holds
```
```bash
...
`npx husky init` sets Git's `core.hooksPath` to `.husky/`, which means the `pre-commit` hook you installed under `.git/hooks/` in step 3 **stops running** — including gitleaks. Make husky the single hook entry point and invoke `pre-commit` from it, so those checks keep firing:

```bash
echo 'pre-commit run' > .husky/pre-commit
```
```bash
...
The `.pre-commit-config.yaml` stays versioned and each teammate still needs `pre-commit` installed locally; husky just becomes the one place hooks are wired.
...
The same thing as code. Push `.github/workflows/checks.yml` and let it run successfully at least once **before** requiring its `pre-commit` check, or the context will not be selectable. JSON is far less error-prone here than nested form arrays:
...
First enable the permission release-please needs. On a new personal repository, GitHub Actions cannot open pull requests by default, so the Release PR will never appear. In the repository, open **Settings → Actions → General → Workflow permissions**, choose **Read and write permissions**, then enable **Allow GitHub Actions to create and approve pull requests**.
...
This course pins `googleapis/release-please-action@v4` for reproducibility. It is not the latest major version (v5.0.0 shipped in April 2026 and moves the runtime to node24); review and deliberately update the pin between course offerings.
...
Create an issue with the Feature issue type (Issues, New issue, then set Type). Issue types require the repository to live in an organization; on a personal repository, apply an `epic` label instead:
