...
* The `module4-release-demo` repository created **in a GitHub organization** in Lab 4C. GitHub's built-in issue types (Feature, Bug, Task) are an organization-only feature and are not available on personal repositories. If your repository is under a personal account, substitute an `epic` label wherever this lab sets the Feature issue type.
