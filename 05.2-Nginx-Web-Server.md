...
Performance tuning in Nginx refers to the process of optimizing Nginx's configuration and settings to improve its speed, efficiency, and ability to handle traffic. 

---

**A) Worker Configuration**

Worker processes handle client requests. `worker_processes` auto matches the number of CPU cores for optimal parallelism. `worker_connections` defines how many simultaneous connections each worker can handle. The `epoll` event mechanism (Linux) efficiently monitors multiple file descriptors without polling overhead. Total `server capacity` = `worker_processes` × `worker_connections`.
   
```nginx
sudo vim /etc/nginx/nginx.conf
```
```nginx
worker_processes auto;        # Match CPU cores (or set specific number)
worker_rlimit_nofile 65536;   # Maximum file descriptors per worker

events {
    # Step 3: Configure connections
    worker_connections 8192;  # Max simultaneous connections per worker
    
    # Step 4: Set event mechanism
    use epoll;                # Linux: epoll, BSD: kqueue, Mac: kqueue
...
#Check resource limits for Nginx process
...
**B) Buffering and Timeouts**

Buffers temporarily store request/response data in memory, reducing disk I/O. Proper buffer sizing prevents memory waste (too large) or excessive disk writes (too small). 

Timeouts prevent slow clients from holding connections indefinitely. Keepalive reuses TCP connections, reducing handshake overhead. Gzip compression reduces bandwidth by 60-80% for textn based content at the cost of CPU cycles.

Configuration Steps:

```nginx
sudo vim /etc/nginx/conf.d/performance.conf
```

```nginx
http {
    # Configure client buffers
    ...
    large_client_header_buffers 4 16k;   # Large header buffer
    ...
    client_header_timeout 12;            # Reading client header
    ...
    gzip_min_length 256;                 # Min size to compress
    
    # Specify gzip MIME types
...
```
 ...
 - The main nginx.conf, it will be automatically included:
```nginx
http {
    include /etc/nginx/conf.d/*.conf;  # This includes performance.conf
}
```

<img width="912" height="547" alt="Screenshot 2026-02-02 at 9 02 42 AM" src="https://github.com/user-attachments/assets/d58fec9b-b9ff-421d-aa3f-207e6985c0ab" />
...
<img width="888" height="167" alt="Screenshot 2026-02-02 at 8 39 46 AM" src="https://github.com/user-attachments/assets/1a96e740-4b14-4806-ae2c-71016e601c99" />

- Verify all the settings
```bash
# Show effective configuration
sudo nginx -T | grep -E "client_body_buffer_size|client_max_body_size|keepalive|gzip"
```
...
# Check comthe pression ratio
curl -H "Accept-Encoding: gzip" http://localhost | wc -c
curl http://localhost | wc -c

# Monitor buffer usage
tail -f /var/log/nginx/error.log | grep "buffer"
```
...
Proxy caching stores backend responses in Nginx, serving subsequent requests from cache without hitting the backend. This dramatically reduces backend load and response times. The levels parameter creates a two level directory hierarchy, preventing too many files in one directory. `keys_zone` defines shared memory for cache keys. Cache is validated using TTL and can serve stale content during backend failures.
...
# This file is automatically included in the http {} block

# Define cache zones
proxy_cache_path /var/cache/nginx/main
                 levels=1:2
                 keys_zone=myy_cache:10m
                 max_size=1g
                 inactive=60m
                 use_temp_path=off;

# Optional: Multiple cache zones for different content types
...
- Server Block with Cache Settings:
...
# Check cache status on requests
curl -I http://localhost | grep X-Cache-Status
...
ls -lh /var/cache/nginx/main/*/*

# Monitor cache hits/misses
tail -f /var/log/nginx/access.log | grep --line-buffered -oP 'X-Cache-Status: \K\w+'

# Check Nginx error log for cache issues
tail -f /var/log/nginx/error.log | grep cache

# View cache zone memory usage
grep -r "cache" /var/log/nginx/error.log
```

---

**D) SSL/TLS Optimization**

SSL/TLS encrypts data between the client and server. TLS 1.2+ provides modern security. Session caching reuses SSL handshake data, reducing CPU overhead by 80-90%. HTTP/2 multiplexes multiple requests over one connection. OCSP stapling allows Nginx to provide certificate validity status, eliminating client OCSP lookups. Proper cipher selection strikes a balance between security and performance.

> Complete example: [05.2.1 HTTPS (SSL-TLS) for Nginx Web Server](https://github.com/techgeek68/DevOps/blob/master/05.2.1%20HTTPS%20(SSL-TLS)%20for%20Nginx%20Web%20Server.md)

---

**E) Static File Serving**

Static files (images, CSS, JavaScript) don't change frequently and can be served with aggressive caching and optimizations:

 - `sendfile`: Uses kernel-level file transmission (zero-copy), bypassing user space for maximum efficiency
 - `tcp_nopush`: Buffers data and sends headers and file data together in one packet, reducing network overhead
 - Browser caching: Reduces bandwidth by storing files locally on the client side
 - `access_log off`: Reduces I/O overhead for static content by disabling logging
 - `sendfile_max_chunk`: Prevents worker processes from blocking on large file transfers
   
> Important: tcp_nopush and tcp_nodelay should NOT be used together as they conflict:
> 
  > `tcp_nopush on`: Buffers data and sends in full packets (better for static files)
> 
  > `tcp_nodelay on`: Sends data immediately without buffering (use only for proxied connections)
...
- Example: Terraform for AWS EC2 + Ansible for Nginx setup.
