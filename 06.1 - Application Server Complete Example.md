...
| **AutoDeploy** | Auto detect and deploy new apps dropped into App base while Tomcat runs |
...
Step 1: Map the Hostname (your machine)

Edit your hosts file to fake a DNS entry:
...
```bash
mkdir -p /opt/tomcat/webapps-mysite/ROOT
```
```bash
echo "<h1>Hello from mysite.local</h1>" > /opt/tomcat/webapps-mysite/ROOT/index.html
```


**Fill the Host Manager Form**

Go to: `http://localhost:8080/host-manager/html`
