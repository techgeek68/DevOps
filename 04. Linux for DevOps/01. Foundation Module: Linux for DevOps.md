...
`vim` (Vi IMproved) is available on virtually every Linux system. It has a learning curve but becomes very fast once you know the basics. It works in different **modes**, which is what trips up beginners.
...
> **The most important thing to remember**: if you are ever stuck in vim, press `Esc` a few times and then type `:q!` to exit without saving.

---
### `nano`: The Beginner Friendly Editor

`nano` is simpler than vim. It shows keyboard shortcuts at the bottom of the screen so you never need to memorize them. It is the better starting point if you are new to terminal editors.
...
Environment variables are key-value pairs that are available to the shell and all processes it starts. They control behavior of the shell and applications.
...
`set -x` enables trace mode, printing each command and its arguments as they are executed. `set -e` causes the script to exit immediately if any command returns a non-zero exit status. `set -u` treats unset variables as errors, preventing silent failures from undefined variables. `set -o pipefail` ensures that a pipeline returns a non-zero status if any command within it fails. These options can be combined as `set -euxo pipefail` at the top of a script for strict error handling. `bash -n` checks a script for syntax errors without executing it, and `bash -v` prints each line as it is read.
...
Let's do a complete hands-on example with `at`.
...
```bash
kill -15 1234               # SIGTERM — graceful shutdown (default, can be caught)
```
```bash
kill -9 1234                # SIGKILL — force kill (cannot be caught or ignored)
```
```bash
kill -1 1234                # SIGHUP — reload config without restarting
```
...
---
> **Always try `SIGTERM (-15)` before `SIGKILL (-9)`** — SIGTERM lets the process clean up files, close connections, and exit properly. SIGKILL cuts it off immediately with no cleanup — can cause data corruption.
...
### 11.5 Setting Resource Limits

**`ulimit`: Persession limits:**
