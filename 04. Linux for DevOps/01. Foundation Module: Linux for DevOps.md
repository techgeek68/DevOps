...
**Note:** on most Linux desktop environments, the agent is auto-started and integrated with the login keyring, so manual `eval "$(ssh-agent -s)"` is mainly needed in headless/server sessions.
