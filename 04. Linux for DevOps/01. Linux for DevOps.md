...
---
### `info`: More detailed documentation for GNU tools
...
Only for numbers, cleanest syntax for math comparisons.
...
---
---
## 9. Process Management, System Monitoring, and Performance Tuning
...
```
<img width="699" height="87" alt="Screenshot 2026-07-01 at 11 49 28 AM" src="https://github.com/user-attachments/assets/95dbc5d4-d22c-4e60-b896-4ff9ca38911f" />

---
- Custom output columns:
> Examples:
```bash
ps -eo pid,ppid,user,%cpu,%mem          # Choose exactly what columns to show
```
- Useful combinations
> Examples:
```bash
...
---
**Additional tools:** Process tree and lookup tools find and target processes by name.
...
```
---
> **Always try `SIGTERM (-15)` before `SIGKILL (-9)`** — SIGTERM lets the process clean up files, close connections, and exit properly. SIGKILL cuts it off immediately with no cleanup, which can cause data corruption.
...
---
```bash
taskset -cp 0 1234          # Pin running process 1234 to CPU core 0
```
> Example:
```
...
---
```bash
taskset -cp <PID>            # Show current CPU affinity of process 1234
```
...
- Persistent: add to `/etc/sysctl.conf` or a file under `/etc/sysctl.d/`:
```
vm.swappiness = 10
...
```
```bash
cpupower frequency-info                    # Show current scaling driver and governor
```
```bash
...
---
*End of Module: Linux for DevOps*
