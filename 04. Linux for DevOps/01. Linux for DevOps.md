...
```
---
> **Why does `postrotate` reload nginx?** After rotation, the old log file is renamed, but nginx still writes to it by file handle. Reloading nginx makes it open the new empty file instead.

> **`delaycompress`** — the most recent rotated log stays uncompressed so you can read it easily without gunzip. Only older ones get compressed.
---
## 8. Process Management, System Monitoring, and Performance Tuning
