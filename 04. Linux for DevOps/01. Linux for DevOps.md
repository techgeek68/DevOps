...
- Check `atd` service is running
```bash
systemctl status atd        # atd must be running for 'at' to work
```

- Schedule interactively
```bash
at now + 1 minute           # open at prompt, runs 1 minute from now
at> echo "Hello from at" >> ~/atjob    # type your command
at> Ctrl+D                  # save and exit (sends the job)
```

- Schedule non interactively
```bash
echo 'echo "Hello" >> /tmp/atlog' | at now + 1 hour    # pipe command directly to at
```
```bash
at -f /path/to/script.sh 09:00 tomorrow                # run a script file at 9 AM tomorrow
```

- Manage scheduled jobs
```bash
atq                         # list all pending jobs with their job IDs
```
```bash
atrm <job_id>               # cancel/remove a scheduled job
```
```bash
at -c <job_id>              # show what a job will run
```
...
| Example | Meaning |
|---------|---------|
| `now + 1 minute` | 1 minute from now |
| `now + 2 hours` | 2 hours from now |
| `now + 3 days` | 3 days from now |
| `09:00 tomorrow` | 9 AM tomorrow |
| `midnight` | tonight at 12 AM |
| `noon` | today at 12 PM |
| `09:00 July 25` | specific date and time |

---
> `at` vs `cron`: Use `at` for one-time tasks, use `cron` for repeating tasks.
