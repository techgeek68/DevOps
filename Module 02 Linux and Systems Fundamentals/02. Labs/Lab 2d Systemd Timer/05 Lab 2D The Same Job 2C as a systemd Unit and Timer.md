...
* Note what is deliberately **absent**: `Persistent=true`. That directive only catches up missed runs on a timer with an `OnCalendar=` directive; on a monotonic `OnBootSec`/`OnUnitActiveSec` timer it does nothing, so leaving it here would only mislead you into thinking missed runs are caught up when they are not.

For a calendar based schedule instead, `OnCalendar=*:0/5` gives you every five minutes on the wall clock. On a calendar timer you can also add `Persistent=true`, which runs a missed job once at the next boot if the machine was off when it was due; the monotonic timer above has no such catch-up, which is why the directive is left off there. Cron has no equivalent without `anacron`. Test any calendar expression before trusting it:
...
Now stop the service on purpose and read the non-OK run through the journal. Note that because of `SuccessExitStatus=0 1` the health-check unit itself still finishes **successfully** here (exit `1` is a valid outcome of the check), so you are reading a `CRITICAL`/`RECOVERED` run, not a failed unit:
