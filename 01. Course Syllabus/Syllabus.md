...
That last shift is not a marketing line. Google's 2025 DORA research, which surveyed nearly five thousand engineers, found that AI adoption raises individual throughput while making team level delivery *less* stable. Change failure rate and rework rate get worse, not better, unless the surrounding delivery system is already strong. AI does not fix a weak pipeline. It amplifies whatever is already there. That finding is the reason this program exists in the shape it does: the review discipline, the automated gates, the observability, and the rollback path are not old fashioned ceremony to get through before you get to the interesting AI part. They are the thing that decides whether the AI part helps you or hurts you.
...
You do not need prior Linux or networking experience. The primer below is the only real prerequisite, and it is designed so a motivated beginner can close the gap before Week 1.
...
Most syllabi hide this. This one will not, because the single fastest way to lose a student is a surprise cloud bill.
...
If you have less than 8 GB, tell the instructor in Week 1. A shared cloud VM can be arranged for the Linux and container work at a cost of a few dollars a month, and that is a normal, supported path, not a favour.

---
## The AI policy, stated plainly

You may use AI assistants on everything in this program. You are expected to. Pretending otherwise in 2026 would be a bad joke, and a graduate who cannot work fluently with an assistant is already behind.

But that changes how you get graded, and it changes it in a way you should understand from day one.

**Every graded artefact is defended live.** Ten minutes, one on one or in front of the cohort. The instructor picks lines from your work and asks you to explain them. Why that CIDR block. What breaks if that readiness probe is deleted. What happens to this rollback if the migration already ran. Why did you choose this over the alternative. If you cannot answer, the artefact does not pass, regardless of whether it works.
...
|---|---|---|
| 1 | Explain the delivery lifecycle, the five DORA metrics, and how DevOps, SRE, and platform engineering relate | Lab 1A, 1D; Checkpoint 1.1 |
| 2 | Administer Linux systems: users, permissions, services, scripting, performance | Labs 2A to 2E; Checkpoint 1.1 |
| 3 | Diagnose and design networks: subnetting, DNS, load balancing, firewalls | Labs 3A to 3C; Checkpoint 1.1 |
...
| 7 | Build, run, and secure containers with Docker and Podman | Labs 6A to 6E; Checkpoint 1.2 |
| 8 | Design CI/CD pipelines in GitHub Actions authenticating via short lived OIDC credentials | Labs 7A to 7F, 14B; Checkpoint 1.2; capstone 4 |
| 9 | Automate configuration with Ansible roles, playbooks, and dynamic inventory | Labs 10A to 10D; capstone 7 |
| 10 | Provision and scan repeatable cloud infrastructure with Terraform or OpenTofu | Labs 9A to 9G; Checkpoint 2.1; capstone 6 |
| 11 | Deploy and operate applications on Kubernetes and on managed EKS | Labs 11A to 11H; Checkpoint 2.2; capstone 8 |
...
| 14 | Implement observability on OpenTelemetry: Prometheus, Grafana, Jaeger, Alertmanager | Labs 13A to 13E; Checkpoint 2.2; capstone 9 |
...
- **The five DORA metrics, current model.** Throughput: deployment frequency, lead time for changes, failed deployment recovery time. Instability: change failure rate, deployment rework rate. Note carefully that the old "mean time to recover" has been renamed and redefined as failed deployment recovery time, which counts only failures caused by a change to production rather than any outage at all, and that rework rate (the share of deployments that are unplanned fixes for a production problem) was added as a fifth metric. If you learned "the four DORA metrics" somewhere else, you learned the 2018 version.
- What the 2025 DORA research found about AI: throughput up, stability down, unless the delivery system underneath is already good. This is the thesis of the entire program and we state it in Week 1 rather than saving it for Module 24.
...
**Checkpoint 1.1 sits at the end of this week.** Three hours, a broken Linux box and a broken network, fix and document.
...
- Make and the Makefile as a build wrapper, so `make test` behaves identically from a laptop, from GitHub Actions, and from Jenkins.
...
**Checkpoint 1.2 closes the course.**

*Nothing in Course 1 requires Kubernetes, a cloud account, or a tool that has not already been taught. Trivy is taught in Module 6 before its first use, and there are no forward references.*
...
- **Cost, taken seriously.** What is actually free tier and what only looks like it. Why a NAT gateway is the most common surprise on a student bill. How to read Cost Explorer. What "destroy your lab" means as a habit rather than a chore.
...
- **8B.** Launch an EC2 instance, install Nginx, confirm it serves traffic, tear it down. Take a screenshot of the empty console.
...
- **10A.** Install Ansible on a control node, write a static inventory with `webservers` and `dbservers` groups, and run ad hoc commands (ping, command, service) before writing a single playbook, so that the gap between a one off command and a repeatable playbook is something you felt rather than read.
...
**Checkpoint 2.1 sits here.** Three hours: a Terraform configuration that will not apply and an Ansible playbook that is not idempotent. Fix both, explain both.
...
- Managed Kubernetes on EKS, and why almost nobody builds their own control plane on AWS any more.
...
- **12E.** Configure an HPA on the application against request rate. Load test it and watch it scale. Then collect real utilisation data, right size the requests and limits, and quantify the monthly saving with Infracost. This is the lab that connects "I set some numbers in a YAML file" to "I saved the company money", and it is the one to put on your CV.
...
- OpenTelemetry as the vendor neutral standard that ties all three together, so you are never locked into one vendor's instrumentation format. Instrument once, export anywhere.
...
- **13D.** Write an alert that fires on a real threshold, route it through Alertmanager to a chat channel, confirm the notification arrives. Then write a *bad* alert that fires constantly, live with it for a day, and write two sentences on what alert fatigue does to an on call engineer.
...
**Checkpoint 2.2 closes the course.**
...
- Incident command: the incident commander, the communications lead, the scribe. Why one person doing all three at 3am is how a one hour incident becomes a four hour one.
...
- Kyverno, policy written as plain Kubernetes resources. OPA Gatekeeper noted for teams already invested in Rego. And a reminder from Module 12: check what Pod Security Admission already does for free before you install either.
...
- **20C.** Install Kyverno. Write a policy that blocks privileged pods. Write a second that requires a verified signature before a pod runs in the production namespace, leaving development unrestricted. Then explain which of these two Pod Security Admission could have done for you without Kyverno at all.
...
- **22C.** Produce a costed rightsizing proposal for your own workload, using the real utilisation data from lab 12E and Infracost. One page, with a number on it, of the kind you could actually hand to a manager.
...
- **What the data says.** The 2025 DORA research found AI adoption improves throughput and worsens delivery stability: change failure rate and rework rate go the wrong way, at team level, even as individuals feel faster. AI amplifies the system it lands in. A team with strong tests, real observability, and a working rollback gets faster. A team without them gets faster at breaking things. Everything in the previous 23 modules is the precondition for this module paying off.
...
- **Chat driven operations.** A language model that can query real observability data and summarise an incident in plain language. Where that genuinely helps an on call engineer at 3am, and where it will confidently summarise an incident that is not the one you are having.
...
- **24E.** Write a threat model for an internal AI assistant with access to your Kubernetes cluster and your team chat. List what it must never be allowed to do unsupervised, and what a prompt injection delivered through a log line or a Jira ticket could achieve if you got the permissions wrong.
...
5. **Getting secrets into the cluster** without putting them in Git. One of Sealed Secrets, External Secrets Operator, or the Vault or OpenBao injector. You must be able to explain why you chose yours.
...
8. **Kubernetes, hardened.** The application runs on EKS (or on the documented free path). RBAC scoped to least privilege for the pipeline service account. Pod Security Admission at `restricted`. A default deny NetworkPolicy with only the required paths opened. Requests, limits, and an HPA.
9. **Observability.** Prometheus and Grafana with a dashboard showing CPU, memory, request rate, error rate, and p95 latency. One working Alertmanager rule that has actually fired. Jaeger tracing the application through OpenTelemetry.
...
**Part 1: Java enterprise (8 hours).** Tomcat: JVM requirements, connector architecture, the Manager app. Maven: the project object model, the build lifecycle, dependency resolution. Lab: create a Maven servlet project, add a dependency, build with `mvn clean package`, verify the WAR, deploy it to Tomcat.
...
|---|---|
| **DVP 101** | Kubernetes and Cloud Native Associate (KCNA), which assumes no prior Kubernetes exposure and is good early groundwork. Linux Foundation Certified System Administrator (LFCS) or CompTIA Linux+ (XK0-006) as the Linux specific route. |
| **DVP 201** | HashiCorp Certified: Terraform Associate. Certified Kubernetes Administrator (CKA). Certified Kubernetes Application Developer (CKAD). AWS Certified Solutions Architect Associate. Along the way: Certified GitOps Associate (CGOA), Certified Argo Project Associate (CAPA), Prometheus Certified Associate (PCA), OpenTelemetry Certified Associate (OTCA). |
...
- **The DORA State of AI-Assisted Software Development report (2025)** and the current metrics guide at dora.dev. Free, current, and the empirical basis for Module 24.
...
Every graded requirement can be met without spending money. You lose realism. You lose no marks. Tell the instructor in Week 1 of Course 2.
...
The one thing the free path cannot give you is the experience of a managed control plane and the shape of a real cloud bill, and those matter. If you can afford 40 to 90 USD across nine months, spend it. If you cannot, do the free path and do not think twice about it.
...
This version fixes what the first version got wrong. The first version was a good content inventory wearing a syllabus costume: it had no schedule, no grading scheme, no cost disclosure, no AI policy, a capstone that would have taken one student a quarter of a platform team's work, and four places where a module quietly depended on a tool taught later. It also taught the 2018 DORA metrics, a deprecated Linkerd API, and a rollback story that would have failed the first time a student ran a migration.

What this version still cannot promise is how it will feel to teach a real, first time cohort. A group moving through Kubernetes at different speeds will surface pacing questions no document predicts. Week 4 of Course 1 is the most likely place for the schedule to slip, and Weeks 9 and 10 of Course 2 are the most likely place for it to slip badly.

Treat this as the strongest version achievable before that first cohort runs through it. Then run the cohort, write down what broke, and revise it again. That is the same loop the course is teaching, and it would be strange to exempt the course from it.
