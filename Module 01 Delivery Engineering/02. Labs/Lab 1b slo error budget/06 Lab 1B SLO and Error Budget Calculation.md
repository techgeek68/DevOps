...
6. The team proposes adding a database query cache to prevent this regression from recurring. Assume the cache reduces database query latency by 120ms on cached requests, but adds approximately 5ms of cache lookup overhead to every request, cached or not. First work out the net latency effect for a cached request (about -115ms) and for a cache miss (about +5ms). Then explain why the effect on the *SLO* cannot be determined from these two numbers alone. State what additional information you would need before you could say whether the SLO improves or regresses, for example the cache hit rate, the baseline latency distribution, and how many requests currently sit just under or just over the 300ms threshold. A strong answer identifies the missing information rather than forcing a single "positive" or "negative" verdict.
...
Numerator: Eligible requests that complete without a server-side failure or timeout (that is, everything in the denominator that did not return a 5xx or time out)
...
For each SLO, calculate the error budget over a 30-day window using the unit that matches the SLO type. Do not express every budget in minutes: only time-based availability budgets are measured in minutes of unavailable time. Request-based SLOs are measured in counts of bad requests, and correctness SLOs in failed or incorrect jobs.

| SLO type | Error-budget unit |
|---|---|
| Time-based availability | Minutes of unavailable time |
| Request success rate | Allowed failed requests |
| Latency compliance | Allowed slow requests (requests over the threshold) |
| Background-job correctness | Allowed failed or incorrect jobs |

| SLO | Error Budget (in the unit matching the SLO type) | What this means in practice |
|---|---|---|
| | | e.g., "This allows approximately one 20-minute outage per month" or "This allows about 12,000 failed requests" |
| | | |
