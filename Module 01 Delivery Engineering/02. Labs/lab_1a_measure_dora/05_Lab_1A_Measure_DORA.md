...
> No terminal is needed for this lab. A calculator and a spreadsheet are enough to inspect public releases and PRs; a GitHub account is only required for the final step where you submit your report to your own repository.
---
### Objective
...
### What You Need
- A GitHub account (free at github.com) — needed only for the final submission step; inspecting public releases and PRs does not require one
...
|---|---|---|
| Deployment Frequency | Public release-frequency proxy: number of GitHub Releases per time window (a release is not necessarily a production deployment) | Moderate |
| Lead Time for Changes | Approximate PR-open-to-public-release elapsed time (not DORA's commit-to-production lead time) | Moderate |
...
| Frequency | Illustrative 2024 DORA Cluster |
|---|---|
...
DORA lead time measures how long code takes to travel from a developer's commit to a production deployment. You cannot see that from outside the project, so in this step you build an *approximation* from PR-open-to-public-release time. Treat the result as a proxy, not as DORA's commit-to-production lead time.
...
3. For each PR, collect:
   ...
   - **Verification (required):** before associating a PR with a release, confirm that the PR's merge commit is actually reachable from the release tag or is explicitly listed in the release notes. Projects use release branches, backports, cherry-picks, delayed merges, and release candidates, so "merged before the release date" does not prove the change shipped in that release. If you cannot confirm it, pick a later release that does include the commit, or exclude the PR from your sample.
...
| Lead Time | Illustrative 2024 DORA Cluster |
|---|---|
...
| DORA Metric | Measured Value | Illustrative 2024 DORA Cluster | Observable from Public Data? |
|---|---|---|---|
| Deployment Frequency | | | Proxy only (public release cadence) |
| Lead Time for Changes | | | Proxy only (PR-to-release, approximate) |
...
| DORA Metric | Measured Value | Illustrative 2024 DORA Cluster | Observable from Public Data? |
|---|---|---|---|
| Deployment Frequency | 1 release every 18 days | Medium | Proxy only (public release cadence) |
| Lead Time for Changes | 13.3 days average | Medium | Proxy only (PR-to-release, approximate) |
| Change Failure Rate | Not measurable | N/A | No |
| Failed Deployment Recovery Time | Not measurable | N/A | No |
| Deployment Rework Rate | Not measurable | N/A | No |
