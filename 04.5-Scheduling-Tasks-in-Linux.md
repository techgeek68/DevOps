...
- Service names:
  ...
  - RHEL/CentOS/Fedora: `crond`
    - Check: `systemctl status crond`
...
- Range: Matches all values from X to Y (inclusive)
  - 1-5: 1, 2, 3, 4, 5
  - Example:
      `0 9 * * 1-5 mycmd                                # Every weekday at 9:00 AM`
    
- Lists: Matches specific values only
  - 1,15: Only 1 and 15
  - Example:
    `0 0 1,15 * * mycmd                                 # Midnight on 1st and 15th of each month`
  
- Steps: Matches every Nth value
  - Pattern A: */N = Every N units (across entire range)
    - */5 in minutes: 0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55
    - Example:
        `*/5 * * * * mycmd                               # Every 5 minutes`
      
  - Pattern B: X-Y/N = Every N units between X and Y
    - 8-18/2: 8, 10, 12, 14, 16, 18
    - Example:
        `0 8-18/2 * * * mycmd                             # Every 2 hours from 8 AM to 6 PM`
...
<img width="850" height="586" alt="Screenshot 2026-01-18 at 7 27 00 AM" src="https://github.com/user-attachments/assets/a62b47bb-2610-4791-9edc-5da0d87b27ee" />
...
```
<img width="749" height="490" alt="Screenshot 2026-01-18 at 7 34 29 AM" src="https://github.com/user-attachments/assets/23ffcb22-04f3-41f3-8cf5-1925cf5a5042" />
...
<img width="747" height="303" alt="Screenshot 2026-01-18 at 7 38 40 AM" src="https://github.com/user-attachments/assets/f4a18896-c294-458f-92d2-e7e785d65c4e" />

  - Drop in directory: `/etc/cron.d/*` (also has “user” field)
<img width="747" height="163" alt="Screenshot 2026-01-18 at 7 42 45 AM" src="https://github.com/user-attachments/assets/b4ce06e0-af7d-4fdc-b8e8-52c5473b0a91" />

- Periodic directories (run by `run-parts`):
  - `/etc/cron.hourly/`
<img width="749" height="507" alt="Screenshot 2026-01-18 at 7 45 08 AM" src="https://github.com/user-attachments/assets/08a4ec16-4b78-496b-804a-937c6a8b0b00" />
...
Note: 
  - `run parts` imposes strict filename rules (no dots by default on many distros). Use hyphens/underscores instead.
...
```
  - PATH:
    - Always set full PATH or use absolute paths for commands.
  - MAILTO:
    - Cron emails stdout/stderr to this address if a local MTA is configured.
  - SHELL:
    - Use bash if your scripts rely on bash-isms.

---

**Timezones and DST((Daylight Saving Time)**
  - Cron typically uses the system timezone.
  - Vixie/modern cron often supports per line `CRON_TZ=Zone/Name` and global `TZ=...` (but not universal across all implementations).

- DST:
  - When clocks change (spring forward / fall back):
      - During spring forward (DST start):
        - The skipped hour means cron jobs scheduled inside that missing hour won’t run.
      - During fall back (DST end):
        - The repeated hour causes cron jobs to run twice; once in each occurrence of that hour.
...
```
```
# Per job timezone (Vixie cron / Debian/Ubuntu often supports)
CRON_TZ=US/Eastern
...
**Avoid Overlaps and Long Runs**
   ...
   - Cap runtime with a timeout to avoid stuck jobs.
...
- At Time Formats:
  - Absolute Time: 
      `14:30 or 2:30 PM                         # Today at that time`
  - Relative Time: 
      `now + 10 minutes/hours/days              # From current time`
  - Specific Date: 
      `09:00 2025-11-30 or 9:00 AM Dec 1        # Exact date & time`
  - Special Keywords: 
      `midnight, noon, teatime                  # Today at standard times`
  - Days / Weeks: 
      `next Mon, Jul 4                          # Specific weekday or date`
  - Combined: 
      `2 PM + 2 days                            # Relative time from a specified hour`
