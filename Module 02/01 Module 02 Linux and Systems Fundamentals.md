...
sudo systemd-run --unit=lab2e-io \
  stress-ng --hdd 2 --hdd-opts sync,wr-seq --hdd-bytes 1G --timeout 3600s
...
sudo systemd-run --unit=lab2e-net \
  bash -c 'for i in $(seq 1 200); do (sleep 3600 < /dev/tcp/127.0.0.1/22 &) ; done; sleep 3600'
```
...
* `await` (or `r_await`/`w_await`), the average time a request spends waiting plus servicing, in milliseconds. On a virtual disk backed by SSD, single digit milliseconds is normal. Tens or hundreds of milliseconds means the queue is deep and requests are stacking up.
